...
{{
    ...
    try:
        ...
        # 返回结构化结果：case_summary 为抽取主体，need_clarify/clarify_question 为追问控制
        return {
    ...
    try:
        ...
        # 注意：此行是一个"裸字典表达式"（bare dict expression），
        # 单独成行不会产生任何效果（既无赋值也无调用），属于遗留的无操作语句，
        # 此处仅按原样保留，不改动逻辑。
        ...
        # 微调收集失败（例如 _ft_output 未定义等）：进入此分支，静默处理
        pass
        # 返回兜底结构（Fallback Structure）：不依赖 LLM 输出，
        # 直接用原始输入字段填出结构一致的 case_summary，保证下游节点照常工作。
        return {
            "case_summary": {
