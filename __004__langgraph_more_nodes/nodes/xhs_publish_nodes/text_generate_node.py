...
# ============================================================
# 文件名称: nodes/text_generate_node.py
# 文件作用: 文案生成
# ============================================================
# 【这个文件是干什么的？】
# 文案生成
...
def generate_xiaohongshu_text(input: str):
    ...
    【容错解析策略】
        LLM 输出的 JSON 经常包含前后缀文本（如 ```json ... ``` 包裹、解释文字等）,
        直接用 parser.parse() 容易失败。这里采用多层容错:
        1. 先尝试用正则提取首个 { } 包裹的 JSON 片段
        2. 提取失败时清理文本中的 markdown 代码块标记再尝试
        3. 仍失败则尝试用 Pydantic 直接解析原文本
        4. 全部失败则抛出异常
    ...
    messages = [
        ...
        SystemMessage(content=(
            ...
            "   - 涵盖法律知识点、常见误区、实用建议\n"
            "   - 可加#话题标签\n"
            "请你严格按照以下格式返回结果，只返回JSON，不要包含任何其他文字或markdown标记：\n"
    ...
    # 【多层容错解析】尝试从 LLM 输出中提取有效的 JSON
    parsed_output = None
    
    # 策略1: 尝试用正则提取 JSON 对象 (处理 LLM 输出中有多余文字的情况)
    ...
    if json_match:
        try:
            ...
            if "title" in parsed_data and "content" in parsed_data:
                parsed_output = XiaohongshuLegalPostOutput(**parsed_data)
                print("  [容错] 策略1成功: 正则提取 JSON 解析")
        except (json.JSONDecodeError, Exception) as e1:
            print(f"  [容错] 策略1失败: {e1}")
    
    # 策略2: 清理 markdown 代码块标记后尝试解析
    if not parsed_output:
        ...
        try:
            parsed_output = parser.parse(cleaned)
            print("  [容错] 策略2成功: 清理 markdown 后解析")
        except Exception as e2:
            print(f"  [容错] 策略2失败: {e2}")
    
    # 策略3: 直接用 Pydantic 解析原文本 (最后尝试)
    if not parsed_output:
        try:
            parsed_output = parser.parse(raw_output)
            print("  [容错] 策略3成功: 直接解析原文本")
        except Exception as e3:
            print(f"  [容错] 策略3失败: {e3}")
    
    # 全部失败则抛出异常
    if not parsed_output:
        raise ValueError(f"无法解析 LLM 输出为结构化 JSON。原始输出前200字: {raw_output[:200]}")
