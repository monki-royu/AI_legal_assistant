...
# ============================================================
# 文件名称: nodes/xhs_auto_publish_node.py
# 文件作用: 自动发布
# ============================================================
# 【这个文件是干什么的？】
# 自动发布
...
#  使用 launch_persistent_context + JSON cookie 管理,
...
def xiaohongshu_auto_publish_node_sync(state: AgentState) -> dict:
    """同步包装器: 供 LangGraph 同步 invoke (graph.invoke) 调用

    【为什么需要】
        xiaohongshu_auto_publish_node 是 async 函数 (内部使用 playwright
        async API + await), 而 LangGraph 主图默认用同步 graph.invoke()
        执行 —— 遇到 async 节点会抛:
            TypeError: No synchronous function provided to "xhs_auto_publish"
        此包装器用 asyncio.run() 为 async 节点起一个独立事件循环, 桥接
        同步/异步两种调用方式:
        - 同步图 (graph.invoke): 直接执行本包装器, asyncio.run 起新 loop;
        - 异步图 (graph.ainvoke): LangGraph 会把同步节点放到线程池执行,
          线程内无运行中事件循环, asyncio.run 依然可安全创建新 loop。

    【注意】
        asyncio.run 每次创建并关闭一个新事件循环, 不允许在已有运行中
        loop 的线程内调用 (会抛 RuntimeError)。本包装器面向 LangGraph
        节点调度 (worker 线程/主线程均无运行中 loop), 用法安全。
    """
    return asyncio.run(xiaohongshu_auto_publish_node(state))
