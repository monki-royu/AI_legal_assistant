...
# 📜 代码文字逻辑解析
# 本文件是 AI 法律助理(LangGraph 多智能体系统)中的"甲乙方识别节点"。
...
如果没有明确名称, party_a 和 party_b 都返回空字符串(""), 不要编造。
...
只输出JSON。"""
        ...
        # 捕获所有异常(LLM 调用失败、JSON 解析失败等)
            ...
            # 打印警告日志
            print(f"  ⚠️ LLM识别失败: {e}")
            # 不编造: 保留规则识别结果(可能为空), 不回填通用名"甲方"/"乙方"
