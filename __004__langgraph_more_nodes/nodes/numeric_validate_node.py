...
【逻辑】本文件是 AI 法律助理(LangGraph 多智能体系统)中的数值校验节点，对应业务流程的 N5c 环节。
    与 N5c-1 数值抽取节点（LLM 抽取）配套使用。核心流程：
...
def _load_rules():
    """
    【功能】内部辅助函数：加载两份 YAML 规则文件，合并为统一的规则列表
    【参数】无
    【返回值】list：合并后的规则列表，每个元素是一个规则字典。若文件不存在则返回空列表。
    【逻辑】① 拼接规则文件目录路径（root_dir + config/rules）
            ② 遍历 contract_review.yaml 和 compliance_review.yaml 两份规则文件
...
def numeric_validate_node(state: AgentState):
    ...
    if not numerics:
        ...
        return state  # 直接返回，避免无意义加载规则

    # ============================================================
    # 【步骤4】加载 YAML 规则文件
    # ============================================================
    # 调用 _load_rules() 加载所有规则，返回规则字典列表
    rules = _load_rules()

    # 打印加载的规则数量，便于调试与审计
    print(f"  加载 {len(rules)} 条数值规则")  # 规则数量日志
