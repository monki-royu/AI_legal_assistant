...
def final_delivery_node(state: AgentState):
    ...
    # 读取甲方名称, 默认"甲方"
    party_a = state.get("party_a", "甲方")
    # 读取乙方名称, 默认"乙方"
    party_b = state.get("party_b", "乙方")
    ...
    # 对风险项按严重程度排序, 缺失 severity 字段时默认权重为 2(中等)
    ...
    # 输出审核模式(如 AI_AUTO / 律师辅助), 缺失时默认 AI_AUTO
    lines.append(f"- **审核模式**: {state.get('review_mode', 'AI_AUTO')}")
...
if __name__ == "__main__":
    ...
    # 调用节点并打印前 500 个字符, 用于人工检查报告格式
    print(final_delivery_node(s)["output"][:500])
