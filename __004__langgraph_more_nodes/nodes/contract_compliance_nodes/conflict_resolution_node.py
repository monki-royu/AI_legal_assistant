...
【逻辑】本文件实现"合规优先原则"的冲突消解逻辑：因为检索结果已做冲突消解
...
def conflict_resolution_node(state: AgentState) -> dict:
    ...
    【新架构 · 双模式自动切换】
    本节点自动根据 state 中的风险项数量判断当前运行模式:
    - 双审模式 (dual): contract_review 任务 → contract_ai_review + compliance_review 均执行
      → contract_risk_items 和 compliance_risk_items 都有值
      → 执行完整的 5 规则冲突消解 (resolve_conflicts)
    - 单审模式 (single): compliance_review 任务 → 仅 compliance_review 执行
      → 只有 compliance_risk_items 有值
      → 跳过冲突消解, 直接使用合规审查结果 + 计算 can_sign

    【参数】state (AgentState)：读取以下字段：
        - contract_risk_items (List[Dict], 可选): 合同审核风险项
        - compliance_risk_items (List[Dict], 可选): 合规审查风险项
        - task_type (str): 任务类型 (用于日志输出)
    写入字段：
    ...
    print(f"--- 开始冲突消解 (task_type={task_type}) ---")
